        ...
        <template #header>
            <div class="flex-nowrap flex justify-between">
                ...
                <el-button v-if="configStore.hasPermission([configStore.create])" type="primary" @click="jump()">
                    {{ configStore.label[configStore.create] }}用户
                </el-button>
            </div>
        </template>
            ...
            <el-table v-loading="loading" :data="list" border>
                ...
                <el-table-column label="昵称" prop="name">
                    <template #default="scope">
                        <div class="flex items-center">
                            <el-avatar :src="scope.row.avatar" class="grow-0 flex shrink-0"/>
                ...
                <el-table-column fixed="right" label="操作" width="120">
                    <template #default="scope">
                        ...
                        <el-popconfirm v-if="configStore.hasPermission(configStore.delete) && scope.row.id != 1"
...
async function destroy(id: number) {
    const result = await UserApi.userDelete(id);
